...
func generateHTML(entries []binaryEntry, config *Config) error {
	...
	funcMap := template.FuncMap{
		...
		"sortByRank": func(entries []binaryEntry) []binaryEntry {
			sortedEntries := append([]binaryEntry(nil), entries...)
	...
	navbarTmplData, err := os.ReadFile(filepath.Join(config.TemplatesDir, "navbar.tmpl"))
	if err != nil {
		return fmt.Errorf("error reading navbar template: %v", err)
	}

	footerTmplData, err := os.ReadFile(filepath.Join(config.TemplatesDir, "footer.tmpl"))
	if err != nil {
		return fmt.Errorf("error reading footer template: %v", err)
	}

	appDetailsTmplData, err := os.ReadFile(filepath.Join(config.TemplatesDir, "app-details.tmpl"))
	if err != nil {
		return fmt.Errorf("error reading app-details template: %v", err)
	}

	appInstallationTmplData, err := os.ReadFile(filepath.Join(config.TemplatesDir, "app-installation.tmpl"))
	if err != nil {
		return fmt.Errorf("error reading app-installation template: %v", err)
	}

	carouselTmplData, err := os.ReadFile(filepath.Join(config.TemplatesDir, "carousel.tmpl"))
	if err != nil {
		return fmt.Errorf("error reading carousel template: %v", err)
	}

	appCardTmplData, err := os.ReadFile(filepath.Join(config.TemplatesDir, "app-card.tmpl"))
	if err != nil {
		return fmt.Errorf("error reading app-card template: %v", err)
	}

	baseTmpl := template.Must(template.New("base").Funcs(funcMap).Parse(string(baseTmplData)))
	baseTmpl = template.Must(baseTmpl.Parse(string(navbarTmplData)))
	baseTmpl = template.Must(baseTmpl.Parse(string(footerTmplData)))
	baseTmpl = template.Must(baseTmpl.Parse(string(carouselTmplData)))
	baseTmpl = template.Must(baseTmpl.Parse(string(appCardTmplData)))
	...
	appTmpl = template.Must(appTmpl.Parse(string(appDetailsTmplData)))
	appTmpl = template.Must(appTmpl.Parse(string(appInstallationTmplData)))
	...
	for _, entry := range entriesWithIcons {
		appDir := filepath.Join(config.OutputDir, entry.Name, fmt.Sprintf("%s:%s", entry.PkgId, entry.RepoName))
	...
	categoryTmplData, err := os.ReadFile(filepath.Join(config.TemplatesDir, "category-index.tmpl"))
	if err != nil {
		return fmt.Errorf("error reading category-index template: %v", err)
	}

	categoryTmpl := template.Must(template.Must(baseTmpl.Clone()).Parse(string(categoryTmplData)))
	...
	for category := range categoryMap {
		...
		if err := categoryTmpl.Execute(categoryFile, data); err != nil {
