(()=>{document.addEventListener("DOMContentLoaded",()=>{let u=document.getElementById("app-details-modal"),v=document.querySelector(".details-body"),g=document.getElementById("image-dialog"),b=document.getElementById("fullscreen-image"),w=document.getElementById("close-dialog");function m(a){a.querySelector(".modal-backdrop").addEventListener("click",()=>{a.close()})}m(u),m(g),window.showAppDetails=function(a,d){if(!Array.isArray(d)){console.error("Apps data is not an array");return}let c=d.find(n=>(n.pkg_name||n.pkg)===a);if(!c){console.error("App not found:",a);return}try{u.showModal(),v.innerHTML=h(c);let n=new URL(window.location);n.searchParams.set("app",a),history.pushState({app:a},"",n)}catch(n){console.error("Error showing app details:",n)}};function h(a){let d=`
        ...
        `;function c(e){let r=e.category?e.category.split(",").map(t=>t.trim()).filter(t=>t).map(t=>`<span class="badge badge-neutral category-tag cursor-pointer" data-category="${t}">${t}</span>`).join(""):"",s=new Date(e.build_date).toLocaleDateString("es-AR",{year:"numeric",month:"short",day:"numeric"}),l=e.rich_description?`
            ...
            `}function n(e){if(!Array.isArray(e)||e.length===0)return Promise.resolve("<p>No screenshots available.</p>");let r=e.map((o,s)=>new Promise(l=>{let t=new Image;t.onload=()=>l({src:o,index:s,valid:!0}),t.onerror=()=>l({src:o,index:s,valid:!1}),t.src=o}));return Promise.all(r).then(o=>{let s=o.filter(t=>t.valid);if(s.length===0)return"<p>No screenshots available.</p>";let l='<div class="carousel w-full h-64 rounded-lg">';return s.forEach((t,i)=>{let f=`slide${i+1}`,p=i===0?s.length:i,k=i===s.length-1?1:i+2;l+=`
                            <div id="${f}" class="carousel-item relative w-full">
                                <img src="${t.src}" class="w-full h-full object-contain cursor-pointer" alt="Screenshot ${i+1}" data-fullscreen-src="${t.src}"/>
                                <div class="absolute left-5 right-5 top-1/2 flex -translate-y-1/2 transform justify-between">
                                    <button data-target="slide${p}" class="btn btn-circle prev-img">\u276E</button>
                                    <button data-target="slide${k}" class="btn btn-circle next-img">\u276F</button>
                                </div>
                            </div>
                        `}),l+="</div>",l})}return v.innerHTML=d,setTimeout(()=>{v.innerHTML=c(a);let e=document.getElementById("screenshots-container");e&&n(a.screenshots).then(r=>{let o=e.querySelector(".skeleton-container"),s=e.querySelector(".carousel-container");if(!s)return console.log("error"),null;s.innerHTML=r,o.remove(),s.classList.remove("hidden"),s.querySelectorAll("img").forEach(l=>{l.addEventListener("click",()=>{b.src=l.dataset.fullscreenSrc,g.showModal()})}),s.querySelectorAll(".prev-img, .next-img").forEach(l=>{l.addEventListener("click",t=>{let i=document.getElementById(l.dataset.target);i&&i.scrollIntoView({behavior:"smooth",block:"nearest",inline:"start"})})})})},1e3),d}});})();
