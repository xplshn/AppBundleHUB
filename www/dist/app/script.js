(() => {
    document.addEventListener("DOMContentLoaded", () => {
        ...
        `; function c(e) {
                let r = e.category ? e.category.split(",").map(t => t.trim()).filter(t => t).map(t => `<span class="badge badge-neutral category-tag cursor-pointer" data-category="${t}">${t}</span>`).join("") : "", s = new Date(e.build_date).toLocaleDateString("es-AR", { year: "numeric", month: "short", day: "numeric" }), l = e.rich_description ? `
            ...
            `} function a(e) {
                if (!Array.isArray(e) || e.length === 0) return Promise.resolve("<p>No screenshots available.</p>"); let r = e.map((i, s) => new Promise(l => { let t = new Image; t.onload = () => l({ src: i, index: s, valid: !0 }), t.onerror = () => l({ src: i, index: s, valid: !1 }), t.src = i })); return Promise.all(r).then(i => {
                    let s = i.filter(t => t.valid); if (s.length === 0) return "<p>No screenshots available.</p>"; let l = '<div class="carousel w-full h-64 rounded-lg">'; return s.forEach((t, n) => {
                        let h = `slide${n + 1}`, f = n === 0 ? s.length : n, p = n === s.length - 1 ? 1 : n + 2; l += `
