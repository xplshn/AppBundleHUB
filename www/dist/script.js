(() => {
    var v = []; function C(u) { v = u } function c() { return v } function $() {
        return `
    ...
    `} document.addEventListener("DOMContentLoaded", async () => {
        ...
        `} function g(e = "") {
                let a = c(); if (e) { let t = e.toLowerCase(); a = a.filter(s => s.pkg_name && s.pkg_name.toLowerCase().includes(t) || s.description && s.description.toLowerCase().includes(t) || s.category && s.category.toLowerCase().includes(t)) } let n = new Set, l = ""; m.forEach((t, s) => {
                    let i = a.filter(r => r.category ? r.category.split(",").map(I => I.trim()).includes(s) && !n.has(r.pkg_name || r.pkg_id) : !1); if (i.length === 0) return; let p = i.slice(0, 6); p.forEach(r => n.add(r.pkg_name || r.pkg_id)); let d = p.map(r => h(r)).join(""); l += `
            ...
            `}), u.innerHTML = l, u.querySelectorAll(".card").forEach(t => { t.addEventListener("click", s => { let i = s.currentTarget.dataset.name; showAppDetails(i, c()) }) }), u.querySelectorAll(".see-more-btn").forEach(t => { t.addEventListener("click", s => { let i = s.target.dataset.category; A(i) }) }); let o = [].slice.call(document.querySelectorAll("img.lazyload")); if ("IntersectionObserver" in window) { let t = new IntersectionObserver(function (s, i) { s.forEach(function (p) { if (p.isIntersecting) { let d = p.target; d.src = d.dataset.src, d.classList.remove("lazyload"), t.unobserve(d) } }) }); o.forEach(function (s) { t.observe(s) }) } else o.forEach(function (t) { t.src = t.dataset.src, t.classList.remove("lazyload") })
                ...
                let n = c().filter(l => l.category ? l.category.split(",").map(t => t.trim()).includes(e) : !1).map(l => h(l)).join(""); u.innerHTML = `
            <div class="category-section my-8">
        ...
        `, u.querySelectorAll(".card").forEach(l => { l.addEventListener("click", o => { let t = o.currentTarget.dataset.name; showAppDetails(t, c()) }) })
            } f.addEventListener("input", e => { let a = e.target.value; g(a); let n = new URL(window.location); n.searchParams.set("search", a), history.pushState({ search: a }, "", n) }), b.addEventListener("input", e => { let a = e.target.value; g(a); let n = new URL(window.location); n.searchParams.set("search", a), history.pushState({ search: a }, "", n) }), w(), window.addEventListener("popstate", e => { let a = new URL(window.location), n = a.searchParams.get("search") || "", l = a.searchParams.get("app") || ""; g(n), l ? showAppDetails(l, c()) : x() }); function x() { let e = document.querySelector(".app-details"); if (e) { e.classList.add("hidden"); let a = new URL(window.location); a.searchParams.delete("app"), history.pushState({}, "", a) } } function S() {
                let e = document.getElementById("popular-apps-carousel"); if (!e) return null; e.innerHTML = `
    ...
    `; let a = c().sort((t, s) => s.popularity_rank - t.popularity_rank).slice(0, 10), n = t => Array.isArray(t.screenshots) && t.screenshots.length > 0 && t.screenshots.some(s => s && s.trim() !== ""), l = a.filter(n); if (l.length === 0) {
                    e.innerHTML = `
                <h2 class="text-2xl font-bold my-4">Trending</h2>
            ...
            `; return
                } let o = l.map((t, s) => L(t, s, l)); Promise.all(o).then(t => {
                    let s = t.join(""); e.innerHTML = `
            <h2 class="text-2xl font-bold my-4">Trending</h2>
            ...
            `, E()
                ...
                let l = e.screenshots.map(o => new Promise(t => { let s = new Image; s.onload = () => t({ src: o, valid: !0 }), s.onerror = () => t({ src: o, valid: !1 }), s.src = o })); return Promise.all(l).then(o => {
                    let t = o.filter(r => r.valid); if (t.length === 0) return null; let s = t[0].src, i = `slide${a + 1}`, p = a === 0 ? n.length : a, d = a === n.length - 1 ? 1 : a + 2; return `
                <div id="${i}" 
